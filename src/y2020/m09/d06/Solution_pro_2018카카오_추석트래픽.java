...
*/
public class Solution_pro_2018카카오_추석트래픽 {
...
//				"2016-09-15 21:00:02.066 2.62s"};
	...
	public static int solution(String[] lines) {
		class Process implements Comparable<Process>{
			...
			int start;
			int end;
			public Process(int no, int start, int end) {
				this.no = no;
				this.start = start;
				this.end = end;
			}
			...
			public String toString() {
				return "Process [no=" + no + ", start=" + start + ", end=" + end + "]";
			}
        ...
        for(int i=0;i<N;i++) {
        	...
        	//day생각
        	al.add(new Process(i,time[i][0],time[i][1]));
        }
        Collections.sort(al);
        for(int i=0;i<N;i++) {
        	int count=0;
        	int startpro = al.get(i).end;
        	int endpro = al.get(i).end+1000;
        	for(int j=0;j<N;j++) {
        		if(al.get(j).end>=startpro&&al.get(j).start<endpro) {
        			count++;
        		}
        	}
        	if(count>answer) {
