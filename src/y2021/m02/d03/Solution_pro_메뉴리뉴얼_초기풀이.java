...
public class Solution_pro_메뉴리뉴얼_초기풀이 {
...
//		String[] orders = {"XYZ", "XWY", "WXA"};
	...
	public static String[] solution(String[] orders, int[] course) {
        ...
        for(int i=0;i<course.length;i++) {
        	...
        	for(int j=0;j<orders.length;j++) {
        		...
        		Arrays.sort(tarr);
        		temp = String.join("", tarr);
        		dfs(menulen,temp,0,0,"");
        	}
        }
	...
	private static void dfs(int menulen, String order,int count,int start,String word) {
		if(count==menulen) {
		...
		for(int i=start;i<order.length();i++) {
			dfs(menulen, order, count+1, i+1, word+order.charAt(i));
		}
	}
}
